...
Write a simple Python function named `edit_image` that edit the user-input image to a jigsaw, and then design 4 different parameter sets.
...
5. Do not use brightness, contrast, saturation, sharpness, and filtering functions.
...
## Example 1 (make a 3*3 jigsaw puzzle):
```python
...
args_list = [
    {'N': 3, 'order': [3, 0, 1, 2, 4, 5, 8, 7, 6]},
    {'N': 3, 'order': [1, 0, 3, 2, 5, 4, 7, 6, 8]},
    {'N': 3, 'order': [0, 5, 6, 7, 1, 8, 2, 3, 4]},
    {'N': 3, 'order': [6, 7, 8, 3, 4, 5, 0, 1, 2]},
]
```

## Example 2 (make a 2*2 jigsaw puzzle):
```python
from PIL import Image
...
2. The Python code must include necessary imports, the `edit_image` function, and a list of dictionaries named `args_list`.
3. Ensure that the 4 sets of parameters in `args_list` produce 4 visually distinct editing results.
4. No comments must be added to your final code.
5. The examples below are for format reference only. The parameters must be designed according to the specific content of the user's image; copying them directly is prohibited.
...
3. Ensure that the 4 sets of parameters in `args_list` produce 4 visually distinct editing results.
4. No comments must be added to your final code.
5. If coordinates are used, use relative coordinates (range 0-1000) in `args_list`. Inside `edit_image`, retrieve the actual image dimensions and convert these relative values to absolute pixel coordinates.
6. The examples below are for format reference only. The parameters must be designed according to the specific content of the user's image; copying them directly is prohibited.
