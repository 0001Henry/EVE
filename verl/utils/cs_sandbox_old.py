...
#TEMP_PROCESSED_IMAGES_DIR = "/data/kcl/hyr/tmp/data/temp_processed_images/" #"/data/ms-swift-latest/eval_o3_tool/tmp_image"
...
ORIGINAL_OUTPUT_PREFIX = '/data/kcl/hyr/tmp/temp_processed_images/'
...
def _execute_sandbox_logic(code_to_execute, input_image_path, temp_output_dir, item_id, 
    ...
    sandbox_globals = {
        ...
        "ImageEnhance": __import__('PIL.ImageEnhance'),
    ...
    try:
        with contextlib.redirect_stdout(captured_stdout):
            original_prefix = '/data/kcl/hyr/tmp/temp_processed_images/'
    ...
    #     # 添加对函数类型的检查，排除用户定义的函数
    #     elif isinstance(value, (types.FunctionType, types.LambdaType)):
    #         # 跳过函数类型，避免 pickle 错误
    #         if random.random() < 0.01:
    #             print(f"Warning: Skipping function '{name}' (cannot be pickled).", file=sys.stderr)
    #         continue
    #     else:
    #         try:
    #             # pickle.dumps(value)
